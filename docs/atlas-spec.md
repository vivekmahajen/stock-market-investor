...
| Scheduled daily universe report with constituent provenance | 20 | **Danelfin**, **Incite AI**, **Magnifi** *(daily picks, opaque ranking)* |
| Horizon price forecasting as a distribution, with measured skill vs a random walk | 21 | **Tickeron**, **Incite AI**, **Prospero.ai** *(point "predictions", no error bar)* |
| Persistent prediction store, resolved outcomes, and a published realised track record | 22 | *(none of the eight publishes a resolved, auditable forecast record)* |
...
The forecasting layer (Sections 20–22) is built on the same candor. It ships a deliberately
plain model — a random walk with a shrunk, capped drift — and then spends most of its effort
proving how little that model knows: a walk-forward check against the naive "price does not
move" baseline, interval coverage that catches bands that are too narrow, and a persistent
store that turns every issued forecast into a resolved hit or miss once its horizon elapses.
On most large, liquid names the honest 30-day verdict is *no measurable edge over a random
walk*, and the report says so rather than burying it. That is the point: a competitor sells
the point forecast, ATLAS sells the interval and the receipts. Rich per-symbol accuracy over
a large resolved sample is also the only foundation on which a genuinely better model could
later be justified — you cannot improve what you have not been scoring.
